...
class CalculatePieceToPalletTest {

    private lateinit var calculatePieceToPallet: CalculatePieceToPallet
    ...
    fun `Aerated concrete piece value to pallet value properly calculated`() {
        ...
        for(i in thicknesses.indices){
            ...
            val result = calculatePieceToPallet(
                piece = piece,
                thickness = thickness
            )
            val expectedResult = (piece * 0.6 * 0.25 * thickness / 100 / cubicMeterInPallet * 100.0).roundToInt() / 100.0
