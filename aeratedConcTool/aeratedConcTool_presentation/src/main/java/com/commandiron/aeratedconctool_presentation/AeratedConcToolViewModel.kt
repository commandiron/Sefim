...
import com.commandiron.aeratedconctool_domain.use_cases.AeratedConcToolUseCases
...
class AeratedConcToolViewModel @Inject constructor(
    private val aeratedConcToolUseCases: AeratedConcToolUseCases,
): ViewModel() {
    ...
    private fun calculateUnitToPallet(){
        if(state.piecePallet.isNotEmpty() &&
            ...
            val result = aeratedConcToolUseCases.calculatePieceToPallet(
    ...
    private fun calculatePalletToUnit(){
        if(state.piecePallet.isNotEmpty() &&
            state.thickness.isNotEmpty()){
            val result = aeratedConcToolUseCases.calculatePalletToPiece(
    ...
    private fun calculateSquareMeterToPallet(){
        if(state.squareMetersPallet.isNotEmpty() &&
                state.thickness.isNotEmpty()){
            val result = aeratedConcToolUseCases.calculateSquareMeterToPallet(
    ...
    private fun calculatePalletToSquareMeter(){
        if(state.squareMetersPallet.isNotEmpty() &&
            state.thickness.isNotEmpty()){
            val result = aeratedConcToolUseCases.calculatePalletToSquareMeter(
    ...
    private fun calculateCubicMeterToPallet(){
        if(state.cubicMetersPallet.isNotEmpty() &&
            state.thickness.isNotEmpty()){
            val result = aeratedConcToolUseCases.calculateCubicMeterToPallet(
    ...
    private fun calculatePalletToCubicMeter(){
        if(state.cubicMetersPallet.isNotEmpty() &&
            state.thickness.isNotEmpty()){
            val result = aeratedConcToolUseCases.calculatePalletToCubicMeter(
    ...
    private fun calculateSquareToCubic(){
        if(state.squareCubic.isNotEmpty() &&
            state.thickness.isNotEmpty()){
            val result = aeratedConcToolUseCases.calculateSquareToCubic(
    ...
    private fun calculateCubicToSquare(){
        if(state.squareCubic.isNotEmpty() &&
            state.thickness.isNotEmpty()){
            val result = aeratedConcToolUseCases.calculateCubicToSquare(
